...
public class MemPlugin extends Plugin {
    ...
    static class MemInfo {
        ...
        //add-being by john
        int pss_total, pss_clean;
        int shared_dirty, private_dirty;
        int shared_clean, private_clean;
        int swapPss_dirty;
        int heap_size, heap_alloc, heap_free;
        //end by john
    ...
    private void generateMemoryInfoSec(Module mod, Chapter mainCh, Section sec) {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            if ("MemTotal".equals(key)) {
                expl = "Total usable ram (i.e. physical ram minus a few reserved bits and the kernel binary code).";
                mTotMem = intValue;
            } else if ("MemFree".equals(key)) {
                expl = "The sum of LowFree+HighFree. This is the amount of yet completely not used memory.";
                mFreeMem = intValue;
            } else if ("Buffers".equals(key)) {
                expl = "Relatively temporary storage for raw disk blocks shouldn't get tremendously large (20MB or so). This memory can be freed by the kernel when needed.";
                mBuffers = intValue;
            } else if ("Cached".equals(key)) {
                expl = "in-memory cache for files read from the disk (the pagecache).  Doesn't include SwapCached. This memory can be freed by the kernel when needed.";
                mCaches = intValue;
            ...
            //add by john
                ...
                expl = "An estimate of how much memory is available for starting new applications, without swapping. Calculated from MemFree, SReclaimable, the size of the file LRU lists, and the low watermarks in each zone. The estimate takes into account that the system needs some page cache to function well, and that not all reclaimable slab will be reclaimable, due to items being in use. The impact of those factors will vary from system to system.";
            }
            //end
    ...
    private void loadServMeminfoSecUnsafe(Module mod_) {
        ...
        while (true) {
            ...
            if (line == null
                    //add by john
                    || line.startsWith("Total PSS by process:")
                    //end by john
                    ) {
            ...
            if (line.startsWith(key)) {
                ...
                if (mode == 'm') {
                    ...
                    //Mod-Begin by John
                        ...
                        memInfo.views = Util.parseInt(line, 21, 30, 0);
                        memInfo.viewRoots = Util.parseInt(line, 52, 60, 0);
                    } else if (line.startsWith("         AppContexts:")) {
                        memInfo.appContexts = Util.parseInt(line, 21, 30, 0);
                        memInfo.activities = Util.parseInt(line, 52, 60, 0);
                    } else if (line.startsWith("              Assets:")) {
                        memInfo.assets = Util.parseInt(line, 21, 30, 0);
                        memInfo.assetManagers = Util.parseInt(line, 52, 60, 0);
                    } else if (line.startsWith("       Local Binders:")) {
                        memInfo.localBinders = Util.parseInt(line, 21, 30, 0);
                        memInfo.proxyBinders = Util.parseInt(line, 52, 60, 0);
                    //Mod-End by John
                    ...
                    //add-begin by john
                        ...
                        memInfo.pss_total = Util.parseInt(line, 16, 22, 0);
                        memInfo.pss_clean = Util.parseInt(line, 25, 31, 0);
                        memInfo.shared_dirty = Util.parseInt(line, 34, 40, 0);
                        memInfo.private_dirty = Util.parseInt(line, 43, 49, 0);
                        memInfo.shared_clean = Util.parseInt(line, 52, 58, 0);
                        memInfo.private_clean = Util.parseInt(line, 61, 67, 0);
                        memInfo.swapPss_dirty = Util.parseInt(line, 70, 76, 0);
                        memInfo.heap_size = Util.parseInt(line, 79, 85, 0);
                        memInfo.heap_alloc = Util.parseInt(line, 88, 94, 0);
                        memInfo.heap_free = Util.parseInt(line, 97, 103, 0);
                    }
                    //end by john
                } else if (mode == 's') {
    ...
    private void generateServMeminfoSec(Module mod_, Chapter mainCh) {
        ...
        Chapter ch = new Chapter(mod.getContext(), "From dumpsys meminfo");
        mainCh.addChapter(ch);
        
        new Block(ch).addStyle("note-box")
            .add("Color coding:")
            .add(new Block().addStyle("level75").add("'Pss Total' is more than 300 MB"))
            .add(new Block().addStyle("level50").add("'Pss Total' is more than 200 MB"))
            .add(new Block().addStyle("level25").add("'Pss Total' is more than 100 MB"));

        // Sort mem info based on pss
        Collections.sort(mMemInfos, new Comparator<MemInfo>() {
            @Override
            public int compare(MemInfo o1, MemInfo o2) {
                return o2.pss_total - o1.pss_total; //mod by john
            }
        });
        ...
        //t.addColumn("SwapPss Dirty (KB)", Table.FLAG_ALIGN_RIGHT);
        ...
        for (MemInfo mi : mMemInfos) {
            String style = "";
            if (mi.pss_total > 300000) {
                style = "level75";
            } else if (mi.pss_total > 200000) {
                style = "level50";
            } else if (mi.pss_total > 100000) {
                style = "level25";
            }
            t.setNextRowStyle(style);
            t.addData(new ProcessLink(mod, mi.pid));
            t.addData(new ShadedValue(mi.pss_total));
            t.addData(new ShadedValue(mi.pss_clean));
            t.addData(new ShadedValue(mi.shared_dirty));
            t.addData(new ShadedValue(mi.private_dirty));
            t.addData(new ShadedValue(mi.shared_clean));
            t.addData(new ShadedValue(mi.private_clean));
            t.addData(new ShadedValue(mi.heap_size));
            t.addData(new ShadedValue(mi.heap_alloc));
            t.addData(new ShadedValue(mi.heap_free));
        }
