...
public class SysPropsPlugin extends Plugin {
    ...
    private static final Pattern UPTIME_PATTERN_1 = Pattern.compile("up time: (.*), idle time: (.*), sleep time: (.*)");
    //add by john
    private static final Pattern UPTIME_PATTERN_2 = Pattern.compile(" (.*) up (.*),  (.*),  load average:(.*)");
    //end
    ...
    //add by john
    private static final Pattern TIME_PATTERN_3 = Pattern.compile("(.*) days*, (.*):(.*)"); //for example: '4 days, 21:13'
    private static final Pattern TIME_PATTERN_4 = Pattern.compile("(.*):(.*)"); //for example: '21:13'
    private static final Pattern TIME_PATTERN_5 = Pattern.compile("(.*) mins*"); //for example: '1 min'
    //end
    ...
    private void loadUptime(BugReportModule br) {
        ...
        Matcher m = UPTIME_PATTERN_1.matcher(line);
        /*if (!m.matches()) {
        ...
        m = UPTIME_PATTERN_2.matcher(line);
        /*if (!m.matches()) {
            br.printErr(4, TAG + "Cannot parse uptime: " + line);
            return;
        }*/
        if (m.matches()) {
            mUpTime = parseTime(br, m.group(2));
            br.setUptime(mUpTime, 100);
            return;
        }
    ...
    private long parseTime(BugReportModule br, String str) {
        ...
        m = TIME_PATTERN_4.matcher(str);
        if (m.matches()) {
            long hours = Long.parseLong(m.group(1));
            long minutes = Long.parseLong(m.group(2));
            return (hours * 60 + minutes) * 60;
        }
        m = TIME_PATTERN_5.matcher(str);
        if (m.matches()) {
