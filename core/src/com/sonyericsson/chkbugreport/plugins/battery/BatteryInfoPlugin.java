...
public class BatteryInfoPlugin extends Plugin {
    ...
    private void genBatteryInfo(BugReportModule br, Chapter ch) {
        ...
        while (idx < cnt) {
            ...
            if (buff.startsWith("Battery History")) {//mod by john
        ...
        if (!foundBatteryHistory) {
            ...
            // Find the timestamp
            long ref = 0l; //getTimestamp(br); //mod by john
            ...
            while (idx < cnt) {
                ...
                //add by john
                //read the reset time.
                m = RESET_TIME_PATTERN.matcher(buff);
                if (ref == 0l && m.matches()) {
                    ...
                    try {
                     cal.set(Calendar.YEAR, Integer.parseInt(sYear));
                        ...
                        br.printErr(4, "Invalid number: '" + sYear + "' in text '" + buff + "'!");
                    }
                    ref = getTimestamp(br, cal);
                }
                //end
                ...
                long ts = 0l;
                try {
                    ts = ref + Util.parseRelativeTimestamp(buff.substring(0, 21)); //mod by john

                    if (ts == ref) continue; //add by john, other unused info by now.
                    // Read the battery level
                    String levelS = buff.substring(26, 29);
   ...
   //add by john
   private long getTimestamp(BugReportModule br, Calendar cal) {
        // First use the provided timestamp
        if (cal != null) {
            // Don't forget the GMT adjustment
            return cal.getTimeInMillis() + br.getContext().getGmtOffset() * HOUR;
        }

        // Then use the bugreport timestamp
        cal = br.getTimestamp();
        if (cal != null) {
            // Don't forget the GMT adjustment
            return cal.getTimeInMillis() + br.getContext().getGmtOffset() * HOUR;
