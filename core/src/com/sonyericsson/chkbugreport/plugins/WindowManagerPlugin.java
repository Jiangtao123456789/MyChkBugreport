...
//        DumpTree.Node root = dump.find(nodeKey, false);
        Section inputSection = br.findSection(Section.DUMP_OF_SERVICE_INPUT);
        if (inputSection == null) {
            br.printErr(3, TAG + "Section not found: " + Section.DUMP_OF_SERVICE_INPUT + " (aborting plugin)");
            return false;
        }

        // Parse the data
        DumpTree input_dump = new DumpTree(inputSection, 0);

        DumpTree.Node root = input_dump.find(nodeKey, false);
        //end
                ...
                for (DumpTree.Node devNode : node) {
                    ...
                    for (DumpTree.Node propNode : devNode) {
                        ...
                        if ("Classes".equals(key)) {
                            dev.classes = value == null ? "" : value;
                        }
                        if ("Path".equals(key)) {
                            dev.path = value == null ? "" : value;
                        }
                        if ("KeyLayoutFile".equals(key)) {
                            dev.kl = value == null ? "" : value;
                        }
                        if ("KeyCharacterMapFile".equals(key)) {
                            dev.kcm = value == null ? "" : value;
                        }
                        if ("HaveKeyboardLayoutOverlay".equals(key)) {
                            dev.overlay = value == null ? "" : value;
                        }
                        if ("Location".equals(key)) {
                            dev.location = value == null ? "" : value;
                        }
                        //end
                    }
                }
    ...
    private void parseWindowAttr(Window win, String attrs) {
        ...
        for (String s : values) {
            if (s.startsWith("fl=")) {//mod by john
    ...
    public void generate(Module br) {
        ...
        //add by john
        generateEventHubChapter(br);
        //end
    }

    //add by john
    public void generateEventHubChapter(Module br) {
        if (mEventHubState == null
                || mEventHubState.devices.size() == 0) return;

        Chapter eventHubCh = br.findOrCreateChapter("Input/Event Hub");

        Table t = new Table();
        eventHubCh.add(t);

        t.addColumn("ID", Table.FLAG_NONE);
        t.addColumn("Name", Table.FLAG_NONE);
        t.addColumn("Classes", Table.FLAG_ALIGN_RIGHT);
        t.addColumn("Path", Table.FLAG_ALIGN_RIGHT);
        t.addColumn("Location", Table.FLAG_ALIGN_RIGHT);
        t.addColumn("KeyLayoutFile", Table.FLAG_ALIGN_RIGHT);
        t.addColumn("KeyCharacterMapFile", Table.FLAG_ALIGN_RIGHT);
        t.addColumn("HaveKeyboardLayoutOverlay", Table.FLAG_ALIGN_RIGHT);
        t.begin();
        for (EventHubState.Device device : mEventHubState.devices) {
            t.addData(device.id);
            t.addData(device.name);
            t.addData(device.classes);
            t.addData(device.path);
            t.addData(device.location);
            t.addData(device.kl);
            t.addData(device.kcm);
            t.addData(device.overlay);
        }
        t.end();
    }
    //end
    ...
    static class EventHubState {
        ...
        static class Device {
            public String location; //add by john
            public String overlay; //add by john
            public String kcm; //add by john
            public String kl; //mod by john
            public String path;
            public String classes; //mod by john
            public String name;
            public int id;
        }
