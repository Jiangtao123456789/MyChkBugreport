...
public class KernelLogData implements LogData {
    ...
    //add by john
    private LogLines mLmkLog = new LogLines();
    //end
        ...
        if (section != null) {
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                if (startIndex > 0 && endIndex > startIndex) {
                    timestamp = Util.strip(line.substring(startIndex + 1, endIndex + 1));
                }*/

                kl = new LogLine(mod, line, LogLine.FMT_KERNEL, null);
                timestamp = kl.getTimeStamp();
                if (line.contains("lowmemorykiller: Killing")) {
                    kl.realTs = -1;
                    if (kl.ok) {
                        mLmkLog.add(kl);
                        type = "lmk";
                ...
                } else if ("lmk".equals(type) && last_timestamp != null
                                && last_timestamp.equals(timestamp)) {
                    mLmkLog.add(kl);
                } else {
                    type = "";
                    last_timestamp = null;
    ...
    private void generateLog(BugReportModule br) {
        ...
        //add by john
        Chapter ch_lmk = new Chapter(br.getContext(), "LMK events");
        mCh.addChapter(ch_lmk);
        new LogToolbar(ch_lmk);
        DocNode log_lmk = new Block(ch_lmk).addStyle("log");

        cnt = mLmkLog.size();
        for (int i = 0; i < cnt; i++) {
            LogLine kl = mLmkLog.get(i);
            log_lmk.add(kl);
        }
        //end
    }
