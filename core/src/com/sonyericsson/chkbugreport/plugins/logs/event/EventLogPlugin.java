...
public class EventLogPlugin extends LogPlugin {
    ...
    protected void generateExtra(BugReportModule rep, Chapter ch) {
        ...
        //add by john
        Chapter ch_wtf = new Chapter(br.getContext(), "WTF warnings");
        ch.addChapter(ch_wtf);
        DocNode log_wtf = new Block(ch_wtf).addStyle("log");

        int cnt = size();
        for (int i = 0; i < cnt; i++) {
            LogLine kl = get(i);
            if ("am_wtf".equals(Util.strip(kl.tag))) {
                log_wtf.add(kl);
            }
        }
